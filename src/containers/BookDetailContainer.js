...
export default function BookDetailContainer() {
  ...
  function getImageCover() {
    let imageUrl
    if (state.data.bookCover.data !== null) {
      imageUrl = ENDPOINT_URL + state.data.bookCover.data.attributes.url
    } else {
      imageUrl = noImage
    }
    return imageUrl
                        ...
                        <div className='max-w-4xl mx-auto space-y-6'>
                          <div className="flex flex-row flex-1">
                            <div className="flex-auto flex justify-center">
                              <img className="object-cover max-w-xs max-h-sm" src={getImageCover()} alt={state.data.bookTitle} style={{ height: "300px", width: '200px', margin: '0 auto' }} />
                            </div>
                            <div className="px-8 flex flex-col flex-2">
                              <h1 className="text-2xl mb-1 font-semibold text-gray-800">{state.data.bookTitle}</h1>
                              <p className="py-4 mb-2 text-gray-400 leading-7">{state.data.bookDesc}</p>
